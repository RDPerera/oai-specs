...
public function main() returns error? {
    ...
    twilio:ListQueueResponse responce = check twilioClient->listQueue(accountSID);

    // Print all queues
    twilio:Queue[]? queues = responce.queues;
    if queues is twilio:Queue[] {
        queues.forEach(function(twilio:Queue queue) {
            io:println(`Name:${queue?.friendly_name} DateTime:${queue?.date_created}  CurrentSize:${queue?.current_size}Calls  MaxSize:${queue?.max_size}Calls`);
        });
    }
